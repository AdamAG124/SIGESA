...
const createWindow = async () => {
  mainWindow = new BrowserWindow({
    // Asignar la ventana creada a mainWindow
    width: 800,
    height: 600,
    webPreferences: {
...
ipcMain.on("listar-usuarios", async (event) => {
  ...
  const usuariosSimplificados = usuarios.map((usuario) => {
    ...
    return {
      ...
      segundoApellidoColaborador: usuario
        .getIdColaborador()
        .getSegundoApellido(),
  ...
  if (mainWindow) {
    // Verifica que mainWindow esté definido
